...
function RequirementsPage() {
  ...
  useEffect(()=>{ fetch(`${API}/requirements`).then(r=>r.json()).then(setReqs) }, [])
  return (
    <section className="page-card">
      <div className="page-header">
        <div>
          <h2>Purchase Requirements</h2>
          <p className="page-description">See the ingredient quantities you need to order based on forecasted production.</p>
        </div>
      </div>
      <div className="table-wrap">
        <table className="data-table">
          ...
          <tbody>
            {reqs.map(r=> <tr key={r.ingredientId}><td>{r.name}</td><td>{r.quantity}</td><td>{r.unit}</td></tr>)}
          </tbody>
        </table>
      </div>
    </section>
